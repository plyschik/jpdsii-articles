            ...
            <div class="col-md-12">
                <div class="box box-primary">
                    <div class="box-header with-border">
                        <h3 class="box-title">{{ __('messages.dashboard.users.edit.header') }}</h3>
                    </div>
                    <form action="{{ route('dashboard.users.edit', ['user' => $user]) }}" method="POST">
                        @method('PATCH')
                        <div class="box-body">
                            <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
                                <label for="emailInput">{{ __('messages.dashboard.users.create.form.email') }}</label>
                                <input class="form-control" id="emailInput" type="text" name="email" value="{{ old('email', $user->email) }}">
                                @if ($errors->has('email'))
                            ...
                            <div class="form-group{{ $errors->has('first_name') ? ' has-error' : '' }}">
                                ...
                                <input class="form-control" id="firstNameInput" type="text" name="first_name" value="{{ old('first_name', $user->first_name) }}">
                            ...
                            <div class="form-group{{ $errors->has('last_name') ? ' has-error' : '' }}">
                                ...
                                <input class="form-control" id="lastNameInput" type="text" name="last_name" value="{{ old('last_name', $user->last_name) }}">
                            ...
                            <div class="form-group{{ $errors->has('role_id') ? ' has-error' : '' }}">
                                ...
                                <select class="form-control select2" id="role_id" name="role_id">
                                    @foreach ($roles as $role)
                                        <option value="{{ $role->id }}" {{ request()->get('role_id', $user->roles->first()->id) == $role->id ? 'selected' : '' }}>{{ $role->name }}</option>
                                    @endforeach
                        ...
                        <div class="box-footer">
                            <input class="btn btn-primary" type="submit" value="{{ __('messages.dashboard.users.edit.form.submit') }}" />
                        </div>
                        @csrf
                    </form>
                </div>
